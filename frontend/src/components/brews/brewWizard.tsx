...
export default function BrewWizard({brew, submit, close, inventory}: {
    brew?: BrewDTO,
    submit: (brew: BrewDTO) => void,
    close?: () => void,
        ...
        <div className="border border-1 p-3 form my-1 bg-body rounded-2" style={{maxWidth: "30rem"}}>
            ...
            <div className="mt-3">
                ...
                <div className="input-group">
                    ...
                    <input className="form-control" type="number" value={newRow.amount}
                           onChange={(e) => setNewRow({...newRow, amount: e.target.valueAsNumber})}></input>
                    <button className="btn btn-success" onClick={() => {
                        setBrewEdit({...brewEdit, usage: [...brewEdit.usage, newRow]});
                        setNewRow({itemId: inventory[0].id, amount: 0})
                    }}>+
                    </button>
                </div>
            </div>
            ...
            <textarea className="form-control" id="comment" value={brewEdit.comment || ""}
